...
const Paragraph: React.FC<ParagraphProps> = ({ children, xs, sm, base, lg, xl, light, medium, semibold, bold, extrabold, center, right, className, black, mainblack, gray, green, lightgray }) => {
    return (
        ...
        ${light ? "font-light" : medium ? "font-medium" : semibold ? "font-semibold" : bold ? "font-bold" : extrabold ? "font-extrabold" : "font-normal"}
         ...
         ${className} ${black ? "text-black" : mainblack ? "text-main-black" : gray ? "text-gray" : green ? "text-green" : lightgray ? "text-light-gray" : "text-white"}
        `}>
            {children}
        </p>
    )
